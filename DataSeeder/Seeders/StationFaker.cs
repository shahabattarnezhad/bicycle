...
{
    private const string KermanName = "کرمان";
    private const double KermanLatitude = 30.2839;
    private const double KermanLongitude = 57.0834;

    public static List<Station> GenerateStations(int count)
    {
        var faker = new Faker("fa");
        var stations = new List<Station>(count);
        ...
        for (int i = 1; i <= count; i++)
        {
            var station = new Station
            {
                ...
                Capacity = faker.Random.Int(5, 20),
                OpenTime = new TimeOnly(6, 0),
                CloseTime = new TimeOnly(22, 0),
                Name = $"ایستگاه {KermanName} شماره {i}",
                Latitude = KermanLatitude + faker.Random.Double(-0.01, 0.01),
                Longitude = KermanLongitude + faker.Random.Double(-0.01, 0.01)
            };
            station.AvailableBicycles = faker.Random.Int(0, station.Capacity);
